...
func main() {
	fmt.Println("MIA - Ejemplo 8, API Rest GO")
...
func ejecucion_comando(commandArray []string) string {
	...
	if data == "crear_disco" {
		...
		respuesta = registrox(commandArray)
	} else {
...
func crear_disco(commandArray []string) string {
	...
	disco, err := os.Create("Ejemplo8.dsk")
	if err != nil {
		msg_error(err)
	}
...
func escribir(commandArray []string) string {
	...
	disco, err := os.OpenFile("Ejemplo8.dsk", os.O_RDWR, 0660)
	if err != nil {
		msg_error(err)
	}
...
func registrox(commandArray []string) string {
	respuesta := "Datos registrados en Archivo Binario: \\n"
	ejm_empty := ejemplo{}
	index := 0
	indexstr := ""
	...
	for i := 0; i < len(commandArray); i++ {
		...
		if strings.Contains(data, ">x=") {
			indexstr = strings.Replace(data, ">x=", "", 1)
			indexstr = strings.Replace(indexstr, "\"", "", 2)
			indexstr = strings.Replace(indexstr, "\r", "", 1)
			indexaux, err := strconv.Atoi(indexstr)
			if err != nil {
				index = 0
				msg_error(err)
			} else {
	...
	lectura := make([]byte, sstruct)
	_, err = disco.ReadAt(lectura, int64(sstruct*index))
	if err != nil && err != io.EOF {
		msg_error(err)
	}
	// Conversion de bytes a struct
